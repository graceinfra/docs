...
2. `{{ .Inputs }}` / `{{ .Outputs }}` (granular control)
   ...
   - **Structure:**
     ...
     - `{{ .DISP }}` _(String)_ : The resolved JCL disposition (e.g. `SHR`, `(NEW,CATLG,DELETE)`). This considers [`job.disp`](/docs/yaml-spec/jobs/zos-specifics#disp) for outputs, and is overridden by [`keep`](/docs/yaml-spec/virtual-paths#keep).
