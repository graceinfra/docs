...
1.  **Grace CLI:**
    - If not, see the [Installation Guide](/docs/installation).
2.  **Zowe CLI**
    ...
    - For help, see the [Grace Configuration Guide](/docs/configuration).
...
# Part 1: Prepare your mainframe data and S3 bucket
...
## Create a sample SDS (sequential data set) on z/OS
...
1. In your terminal, navigate to where you want to create your project and run:
   ...
   ```bash
   grace init my-sds-to-s3-flow
   ```

2. Follow the interactive prompts:

   - **Workflow name:** You can accept the default (`my-sds-to-s3-flow`) or change it.
...
3. Enter your new project directory:
   ...
   ```bash
   cd my-sds-to-s3-flow
   ```

---

# Part 2: Configuring `grace.yml`

Open the `grace.yml` file that `grace init` created in your `my-sqds-to-s3-flow` directory. We'll modify the [`config`](/docs/yaml-spec/config) and [`datasets`](/docs/yaml-spec/datasets) sections, and then define our single [`shell`](/docs/yaml-spec/jobs/shell) job.
  ...
  - name: SDSTOS3
    ...
    with:
      inline: |
        ...
        S3_KEY="migrated_data/sds_extract_$(date +%Y%m%d_%H%M%S).txt"
...
1. `name: SDSTOS3`: A clear name for this job
...
datasets:
  jcl: "USER01.MYSDS2S3.JCL"
  src: "USER01.MYSDS2S3.SRC"
  loadlib: "USER01.LOAD"
...
# Part 4: Running the Grace workflow
...
1. **Ensure you are in your project directory** (e.g. `my-sds-to-s3-flow`) in your terminal session.
...
2. **Execute the workflow**
   ...
   You can also add the `-v` or `--verbose` glag for more detailed output, which is helpful for troubleshooting:

   ```bash
   grace run -v
   ```

As Grace runs the `SDSTOS3` job, note key steps in the output:

- Grace launching the `SDSTOS3` job (🚀).
...
1. **Check AWS S3:**
   ...
   You should see your uploaded file, e.g. `sds_extract_YYYYMMDD_HHMMSS.txt`.

2. **Inspect the uploaded file:**

   ```bash
   aws s3 cp s3://grace-demo-bucket/migrated_data/sds_extract_YYYYMMDD_HHMMSS.txt ./downloaded_from_s3.txt
...
3. **Review Grace logs** (optional):
   ...
   - You'll find:
     - `SDSTOS3_shell-xxxxxx.json`: The detailed JSON log for your shell job, containing the `stdout` and `stderr` from your inline script.
