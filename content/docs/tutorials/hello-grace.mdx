...
Let’s cover the basics of the Grace CLI and get **your first workflow up and running**. We’ll start with a simple CI-style pipeline that automates compiling, linking, and executing a “Hello, Grace” COBOL program on z/OS.
...
The recommended way to create a new Grace workflow is with [grace init](../reference/cli-reference/init). This scaffolds the necessary subdirectories and starter files.
...
1. **Workspace name**: Leave this blank to use the current directory or populate it to create a new directory
...
Grace uses [Zowe](https://www.zowe.org/) for authentication. If you work with z/OS systems, you might already have this configured. If not, see [the official docs](https://docs.zowe.org/stable/web_help/docs/zowe_config_init.html).
...
See the [YAML spec](/docs/category/yaml-spec) for more information on `grace.yml` fields.
...
To dive deeper, see [YAML Spec - Jobs](/docs/reference/yaml-spec/jobs) for job types, configuring dependencies and concurrency, and advanced chaining techniques.

> Prefix `temp://` is a platform agnostic virtual mount path. It denotes intermediate datasets between jobs that are handled behind the scenes by Grace. The engine will provision datasets, wire dependencies, and handle cleanup automatically.
>
> In a production workflow, you might have virtual `temp://` paths holding data between mainframe operations, cloud endpoints, local scripts, and so on.
>
> To learn more about how this works, see LINK TODO HERE.
...
> Command `grace deck` is **idempotent**. You can run it repeatedly to sync local workflow files to the mainframe without triggering job execution.
...
> If you are curious about what orchestration looks like in a hosted or asynchronous environment, see [grace submit](/docs/reference/cli-reference/submit).

> Command `grace run` does not upload or regenerate anything. It purely handles workflow orchestration and monitoring with the last datasets you decked.
...
**Welcome to modern DevOps on the mainframe.** You're on your way to building **automation, pipelines, and tooling on your own terms** with the same principles you'd use to deploy a container or cloud function.
