...
```yaml
jobs:
  - name: LOCALSETUP # Example: A shell job doing some local setup
    type: shell
    with:
      inline: |
  ...
  - name: PROCESSDATA # Example: A shell job processing a downloaded mainframe file
    type: shell
    depends_on: [SOME_MAINFRAME_JOB_THAT_PRODUCES_RAWDATA] # Assuming this input comes from z/OS
    with:
      script:
        ./scripts/process_mainframe_data.sh
        # Script uses $GRACE_INPUT_RAWDATA
        # and writes to $GRACE_OUTPUT_PROCESSED
    inputs:
...
```yaml
inputs:
  - name: CONFIG_FILE # Exposed as $GRACE_INPUT_CONFIG_FILE
    path: zos://PROD.PARMLIB(APPCONF)
  - name: LOCAL_DATA # Exposed as $GRACE_INPUT_LOCAL_DATA
    path: file://./input-data.csv
```
