...
This page details these common parameters. For fields applicable to _all_ job types (including `shell`), such as `name`, `type`, `depends_on`, `inputs`, and `outputs`, refer to [YAML Spec - Jobs](/docs/yaml-specification/jobs).

For detailed behavior on how these parameters are used by specific z/OS job modules, see the individual module pages:

<Cards>
  <Card href="/docs/yaml-specification/jobs/compile" title="Compile">
  ...
  <Card href="/docs/yaml-specification/jobs/linkedit" title="Linkedit">
  ...
  <Card href="/docs/yaml-specification/jobs/execute" title="Execute">
...
- **Description:** Allows you to override the global default settings for `compiler` or `linker` toolchains for this specific job. This is useful when a particular compilation or link-edit step requires different parameters, program names, or `STEPLIB`s than those defined in the global [`config.defaults`](/docs/yaml-specification/config#defaults) section.
- **Sub-fields:**
  - `compiler` _(Object, Optional)_ : Overrides for compiler settings. Any fields specified here will take precedence over [`config.defaults.compiler`](/docs/yaml-specification/config#defaults) for this job.
  ...
  - `linker` _(Object, Optional)_ : Overrides for linkage editor settings. Any fields specified here will take precedence over [`config.defaults.linker`](/docs/yaml-specification/config#defaults) for this job.
