...
Grace executes these jobs based on their defined order, dependencies, and types, orchestrating operations across z/OS and other platforms (like the orchestrating machine for `shell` jobs).
...
For a full explanation of virtual path prefixes and how data flows between jobs and platforms, see [Virtual Paths and Data Flow](#).
...
- **Description:** Allows you to override the global default settings for compiler or linker toolchains for this specific job. This is useful when a particular compilation or link-edit step requires different parameters, program names, or `STEPLIB`s than those defined in the global config.defaults section.
- **Sub-fields:**
  - `compiler` _(Object)_ : Overrides for compiler settings. Any fields specified here will take precedence over config.defaults.compiler for this job.
  ...
  - `linker` _(Object)_ : Overrides for linkage editor settings. Any fields specified here will take precedence over `config.defaults.linker` for this job.
...
- **Behavior:**
  - If a field (e.g. `parms`) is specified within `job.overrides.compiler`, it will be used.
  - If it's not specified in `job.overrides.compiler`, Grace will then look at `config.defaults.compiler`.
  - If it's also not in `config.defaults.compiler`, Grace may use an internal hardcoded default (e.g. for `PGM` names like `IGYCRCTL`), though this should avoided.
    Example: Overriding compiler parameters and specifying a unique STEPLIB for one compile job.
