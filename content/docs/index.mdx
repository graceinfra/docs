...
Grace is an **open source orchestration toolchain** for cross-platform mainframe and cloud automation.

Built for migration consultants and mainframe engineers alike, Grace abstracts JCL and z/OS complexity behind a **declarative YAML workflow format**, allowing you to define, run, and monitor cross-platform workflows the same way you'd ship containers or automate CI/CD.
...
Grace is named in honor of [Grace Hopper](https://en.wikipedia.org/wiki/Grace_Hopper) — COBOL pioneer and one of the first to assert that computer interfaces should be _human_. We like to think she'd be excited to see her ideas remixed with today's infrastructure philosophies and applied to the world's most revered computing platform.
