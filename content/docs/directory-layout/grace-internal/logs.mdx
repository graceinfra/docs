...
- **Content:** This file contains detailed, timestamped log entries (at DEBUG level by default) covering the entire lifecycle of the background orchestration:
...
- **Purpose:** This is the primary log for diagnosing issues with workflows run via grace submit, as there is no direct terminal output from the background process.
