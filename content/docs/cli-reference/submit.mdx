...
- **`grace status <workflow-id>` (Future):** A command to query the status of a submitted workflow is a planned feature.
- **`grace dash` (Future):** A terminal-based dashboard for viewing active and completed workflows is also envisioned.
